...
using System.Linq;
using UDPServerAndWebSocketClient;
...
namespace UDPServerAndWebSocketClient
{
    public class DataMessageWithKey : DataMessage, ILoraMessage
    {
        ...
        private (bool, string, byte[]) DecryptFRMPayload()
        {
            ...
            for (var block = 0; block < blocks; block++) //block =2. plainS.length=32;
            {
                var Ai = A(block);// 16byte
                plain_S.AddRange(Ai);
            }

            var cipherstream = new AES(key).Encrypt(plain_S.ToArray()); // Aes 128
        ...
        public string GetFRMPayLoadDecryptedString()
        {
            return DecryptFRMPayload().Item2;
        }

        public byte[] GetFRMPayLoadDecrypted()
        {
            return DecryptFRMPayload().Item3;
        }
